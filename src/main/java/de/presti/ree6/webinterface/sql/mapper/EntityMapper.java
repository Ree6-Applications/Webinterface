package de.presti.ree6.webinterface.sql.mapper;
...
public class EntityMapper {
    ...
    public SQLResponse mapEntity(StoredResultSet resultSet, Class<?> clazz) {
        try {
            ...
            Server.getInstance().getLogger().error("Couldn't map Entity: " + clazz.getSimpleName(), e);
        }
    ...
    private Object mapClass(StoredResultSet.StoredData resultSet, Class<?> clazz) {
        try {
            ...
            Server.getInstance().getLogger().error("Couldn't map Class: " + clazz.getSimpleName(), exception);
        }
    ...
    public void setAllFields(StoredResultSet.StoredData resultSet, Object entity, Field[] fields, Class<?> clazz) {
        for (Field field : fields) {
            ...
            try {
                ...
                Server.getInstance().getLogger().error("Could not set field " + field.getName() + " of class " + clazz.getSimpleName(), e);
            }
        }
    }
