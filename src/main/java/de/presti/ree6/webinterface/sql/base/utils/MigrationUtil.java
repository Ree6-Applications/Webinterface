package de.presti.ree6.webinterface.sql.base.utils;

import de.presti.ree6.webinterface.Server;
import de.presti.ree6.webinterface.sql.SQLConnector;
import de.presti.ree6.webinterface.sql.migrations.Migration;
...
public class MigrationUtil {
    ...
    public static void runMigration(File file, SQLConnector sqlConnector) throws IOException {
        ...
        if (sqlConnector.querySQL("SELECT * FROM Migrations WHERE NAME=?", migration.getName()).hasResults()) {
            Server.getInstance().getLogger().info("Migration {} already ran.", migration.getName());
            return;
        }

        migration.up(sqlConnector);
        Server.getInstance().getLogger().info("Migration {} ran.", migration.getName());
    }
    ...
    public static void saveMigration(Migration migration) {
        Server.getInstance().getLogger().info("Saving Migration: {}", migration.getName());
        ...
        try (PrintWriter printWriter = new PrintWriter(file)) {
            ...
            Server.getInstance().getLogger().info("Could not save migration, reason: " + exception.getMessage(), exception);
        }
    }
