...
public class SQLConnector {
    ...
    public SQLConnector(String databaseUser, String databaseName, String databasePassword, String databaseServerIP, int databaseServerPort) {
        ...
        try {
            ...
            Server.getInstance().getLogger().error("Error while running Migrations!", exception);
        }
    ...
    public void createTables() {
        ...
        for (Class<? extends SQLEntity> aClass : classes) {
            ...
            try {
                ...
                // Notify if there was an error.
                Server.getInstance().getLogger().error("Couldn't create " + aClass.getSimpleName() + " Table.", exception);
            }
        }
    }
    ...
    public StoredResultSet querySQL(String sqlQuery, Object... objcObjects) {
        ...
        try {
            ...
            if (connectedOnce()) {
                Server.getInstance().getLogger().error("Couldn't send Query to SQL-Server, most likely a connection Issue", exception);
                connectToSQLServer();
                return querySQL(sqlQuery, objcObjects);
            }
        } catch (Exception exception) {
