...
public class LevelUtil {
    ...
    public static int getCurrentRank(UserLevel userLevel) {
        if (userLevel instanceof ChatUserLevel) {
            return Server.getInstance().getSqlConnector().getSqlWorker().getAllChatLevelSorted(userLevel.getGuildId()).indexOf(userLevel.getUserId()) + 1;
        } else if (userLevel instanceof VoiceUserLevel) {
            return Server.getInstance().getSqlConnector().getSqlWorker().getAllVoiceLevelSorted(userLevel.getGuildId()).indexOf(userLevel.getUserId()) + 1;
        }
